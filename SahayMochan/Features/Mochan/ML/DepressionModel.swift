...
final class DepressionModel {
    ...
    func predict(questionnaireScore: Int, auFrames: [AUFrame]) -> Double {
        do {
            ...
            return scale(rawOutput: rawOutput, maximumScore: 27)
        } catch {
            print("DepressionModel inference failed: \(error.localizedDescription). Using fallback.")
            return fallbackPrediction(questionnaireScore: questionnaireScore, auFrames: auFrames, maximumScore: 27)
        }
    }
    ...
    private func makeInterpreter(modelPath: String) throws -> Interpreter {
        ...
        do {
            ...
            print("DepressionModel interpreter creation failed with options for path \(modelPath): \(error)")
